...
struct ProtoAddressInitialiser {
    void operator() (ProtoAddress* msg) {
    }
};

struct ProtoAddressReleaser {
    void operator() (ProtoAddress* msg) {
    }
};
 ...
 */
template <class T, int SZ, class Initialiser, class Releaser> class MgenPool {
public:
    using pointer = std::unique_ptr<T,std::function<void(T*)> >;
    using PoolType = MgenPool<T,SZ,Initialiser, Releaser>;

    MgenPool() {}
    MgenPool(const PoolType& orig) = delete;
    ~MgenPool() {}
